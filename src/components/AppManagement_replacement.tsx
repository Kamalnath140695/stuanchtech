    ...
    try {
      ...
      await apiFetch('/api/apps/container-permission', {
        ...
        body: JSON.stringify({
          ...
          userPrincipalName: trimmedUPN || undefined,
          role: assignRole,
        }),
      });
      setSuccess(
        trimmedUPN
          ? `Success: App access was granted and ${trimmedUPN} was assigned to the selected container.`
          : 'Success: App access was granted for the selected container type.'
      );
      logAudit('ASSIGN_APP_CONTAINER', selectedApp.displayName, `containerId: ${selectedContainerId}, UPN: ${trimmedUPN || 'none'}, role: ${assignRole}`);
      setAssignOpen(false);
      setError('');
      loadAssignmentsSummary();
                  ...
                  <TableRow>
                    ...
                    <TableCell>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        ...
                        {(can('deleteApp') || can('manageAllApps')) && (
                          <Button
                            size="small"
                            appearance="subtle"

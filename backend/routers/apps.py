    ...
    if user_upn:
        ...
        if permission_res.status_code not in (200, 201):
            try:
                ...
                # Detect SPE access denied
                if err_body.get("error", {}).get("innerError", {}).get("code") == "speAccessDenied":
                    raise HTTPException(
                        status_code=permission_res.status_code,
                        detail={
                            "code": "SPE_ACCESS_DENIED",
                            "message": "SharePoint Embedded permissions insufficient for user permission grant",
                            "detail": str(err_body),
                            "contactAdmin": True
                        }
                    )
